...
def formatar_nome(df, coluna, novo_nome="ds_formatada"):
    ...
    df[novo_nome] = df[novo_nome].replace({"CAMPINAGRANDEMIXINGCENTER": "CAMPINA GRANDE"})
    return df
...
df_dtb = formatar_nome(df_dtb, coluna="ds_mun")
data = formatar_nome(df_wide_valores, coluna="ds_mun")
# === AGRUPAR para juntar duplicados (ex: CAMPINA GRANDE + MIXING CENTER) ===
cols_non_numeric = ['ds_formatada', 'sg_uf']  # chaves de agrupamento
cols_numeric = [c for c in data.columns if c not in cols_non_numeric]

data = (
    data.groupby(['ds_formatada', 'sg_uf'], as_index=False)[cols_numeric]
...
# o instituto so atua em escola municipal 
ideb = ideb.query("rede == 'Municipal'").reset_index(drop=True)
